...
// Also asserts BUNNY_API_KEY is present when the gate is open, since
...
if (E2E_ENABLED) {
  if (!process.env['BUNNY_API_KEY']) {
    ...
    throw new Error(
      'BUNNY_E2E=1 requires BUNNY_API_KEY to be set. Refusing to run e2e against an unauthenticated client.',
    );
  }
}
