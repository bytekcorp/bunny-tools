...
**Binary:** `bunny`  |  **Version:** 0.1.0-rc.54  |  **Active commands:** 55/56
...
- `bunny configure` [active] — Interactive credential walkthrough for a profile (API key + optional storage/stream).
...
- `bunny dns record add` [active] — Add a DNS record (A, AAAA, CNAME, TXT, MX, REDIRECT, PULLZONE, SRV, CAA, PTR, SCRIPT, NS). _mcp: `bunny.dns_record_set`_
...
- `bunny stream library delete` [active] — Delete a Stream video library.
...
- `bunny install mcp` [active] — Register bunny-tools as an MCP server in Claude Code (one-shot bootstrap).
- `bunny update` [active] — Self-update bunny-tools to the latest version on npm.
